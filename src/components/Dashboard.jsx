...
const Dashboard = ({ userTokens, gameStats, onTabChange }) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-neon-red mb-2">Dashboard</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat, index) => {
          ...
          return (
            <div key={index} className="bg-space-light/50 rounded-lg p-4 border border-red-600 retro-card">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-gray-400 text-sm">{stat.label}</p>
                  <p className="text-2xl font-bold text-neon-red">{stat.value}</p>
                </div>
      ...
      {/* Quick Actions */}
      <div>
        <h3 className="text-xl font-semibold text-neon-red mb-4">Quick Actions</h3>
          ...
          {quickActions.map((action, index) => {
            ...
            return (
              <button
                ...
                className="bg-space-light/50 rounded-lg p-6 border border-red-600 hover-glow transition-all text-left group retro-card"
              >
                <div className={`${action.color} w-12 h-12 rounded-lg flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}>
                  <Icon className="text-white" size={24} />
                </div>
                <h4 className="text-lg font-semibold text-neon-red mb-2">{action.title}</h4>
      ...
      {userTokens.length > 0 && (
        <div>
          <h3 className="text-xl font-semibold text-neon-red mb-4">Your Tokens</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {userTokens.slice(0, 4).map((token, index) => (
              <div key={index} className="bg-space-light/50 rounded-lg p-4 border border-red-600 retro-card">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="text-lg font-semibold text-neon-red">{token.name}</h4>
                  <span className="text-sm text-gray-400">{token.symbol}</span>
                </div>
                <p className="text-gray-400 text-sm mb-2">{token.description}</p>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-400">Supply:</span>
                  <span className="text-neon-red">{token.totalSupply?.toLocaleString()}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {userTokens.length === 0 && (
        <div className="bg-space-light/30 rounded-lg p-6 border border-red-600 retro-card">
          <h3 className="text-xl font-semibold text-neon-red mb-4">Getting Started</h3>
